...
@interface ViewController ()<Part1_TableViewCellDelegate>
...
- (void)viewDidLoad {
    ...
    YSegleHand *seg = [YSegleHand sharedSegleHandle];
    [seg initDataBase];
...
//    NSLog(@"mainCellArray : %@",mainCellArray);
    ...
    moveView = [[MoveView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 70)];
    moveView.backgroundColor = [UIColor grayColor];
    webView = [[YWebView alloc]initWithFrame:CGRectMake(self.view.frame.size.width, 0, 0, self.view.frame.size.height)];
    ...
    [moveView.longTableView registerClass:[Part2_TableViewCell class] forCellReuseIdentifier:@"cellM"];
